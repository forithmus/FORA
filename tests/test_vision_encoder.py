...
class TestResidualTemporalDownsample:
    ...
    def test_output_shape_standard(self, cnn):
        """Standard CT: [B, 3, 128, H, W] -> [B, 3, 32, H, W] (4x temporal downsample)."""
    ...
    def test_residual_connection(self, cnn):
        ...
        # The residual connection means output should be non-zero even for random input
        out = cnn(x)
...
class TestVJEPA2EncoderStructure:
    ...
    def test_forward_cnn_depth_repeat(self):
        ...
        x = torch.randn(1, 3, 1, 16, 16)  # single slice X-ray
    ...
    def test_forward_cnn_channel_repeat(self):
        ...
        x = torch.randn(1, 1, 8, 16, 16)  # grayscale CT
        input_channels = 3
    ...
    def test_forward_cnn_xray_full_pipeline(self):
        """Full X-ray pipeline: [B, 1, 1, H, W] -> depth repeat -> channel repeat -> CNN."""
        cnn = ResidualTemporalDownsample(in_channels=3)
        input_channels = 3

        x = torch.randn(2, 1, 1, 32, 32)  # batch of X-rays
