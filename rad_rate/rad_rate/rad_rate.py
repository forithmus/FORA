...
class RADRATE(nn.Module):
    ...
    def forward(
        ...
        if debug and dist.is_initialized():
            print(f"[DEBUG RADRATE rank={dist.get_rank()}] forward start: image={image.shape}", flush=True)
        ...
        # 1. Encode Text
        if debug and dist.is_initialized():
            print(f"[DEBUG RADRATE rank={dist.get_rank()}] encoding text...", flush=True)
        ...
        if debug and dist.is_initialized():
            print(f"[DEBUG RADRATE rank={dist.get_rank()}] text done, encoding visual...", flush=True)
        ...
        if debug and dist.is_initialized():
            print(f"[DEBUG RADRATE rank={dist.get_rank()}] visual done: tokens={visual_tokens.shape}, mask={token_mask.shape}", flush=True)
        ...
        if return_loss:
            ...
            if debug and dist.is_initialized():
                print(f"[DEBUG RADRATE rank={dist.get_rank()}] starting all_gather for text_latents...", flush=True)

            if dist.is_initialized():
                text_latents_all = all_gather_batch(text_latents)
                if debug:
                    print(f"[DEBUG RADRATE rank={dist.get_rank()}] text_latents gathered: {text_latents_all.shape}", flush=True)
                visual_tokens_all = all_gather_batch(visual_tokens)
                if debug:
                    print(f"[DEBUG RADRATE rank={dist.get_rank()}] visual_tokens gathered: {visual_tokens_all.shape}", flush=True)
                # GATHER TOKEN MASK
                token_mask_all = all_gather_batch(token_mask.float()).bool()
                if debug:
                    print(f"[DEBUG RADRATE rank={dist.get_rank()}] token_mask gathered: {token_mask_all.shape}", flush=True)
                ...
                if sentence_mask is not None:
                    ...
                    mask_all = None
                if debug:
                    print(f"[DEBUG RADRATE rank={dist.get_rank()}] all_gather complete, computing logits...", flush=True)
            else:
